...
export default function Sidebar({ userRole = "member" }: Props) {
  ...
  // Sembunyikan "Get Started!" jika setup sudah selesai/dismissed
  const [setupDone, setSetupDone] = useState(false);

  useEffect(() => {
    try {
      setSetupDone(localStorage.getItem("maalify_setup_dismissed") === "true");
    } catch { /* ignore */ }

    // Dengarkan perubahan saat SetupChecklistWidget auto-dismiss
    function onStorage(e: StorageEvent) {
      if (e.key === "maalify_setup_dismissed" && e.newValue === "true") setSetupDone(true);
    }
